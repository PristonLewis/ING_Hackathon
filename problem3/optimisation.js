  ...
  app.update = function (timestamp) {
    ...
    for (var i = 0; i < app.count; i++) {
      ...
      if (!app.optimize) {
        ...
        var topL = m.style.top.replace('px', '');
        var leftL = m.style.left.replace('px', '');
        var top = m.classList.contains('down') ? topL + distance : topL - distance;
        var left = m.classList.contains('right') ? leftL + distance : leftL - distance;
        if (top < 0) top = 0;
        if (top > maxHeight) top = maxHeight;
        m.style.top = top + 'px';
        if (left < 0) left = 0;
